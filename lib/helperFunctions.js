...
const getAllContributions = (story_id) => {
  return pool
          .query(`SELECT contributions.id, content, COUNT(upvotes.*) FROM contributions JOIN upvotes ON upvotes.contribution_id = contributions.id WHERE story_id = $1 GROUP BY contributions.id;`, [story_id])
          .then((contributions) => {
...
module.exports = { getAllStories, getMyStories, getStoryById, createNewStory, getAllContributions, createNewContribution, addUpvote }
